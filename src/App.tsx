...
function PaymentCallbackPage() {
  ...
  const amountLabel = amount != null && !Number.isNaN(amount)
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-10 max-w-md w-full text-center">
        ...
        <p className="text-gray-500 mb-4">
          {amountLabel
            ? <>Your wallet has been credited with <span className="font-bold text-gray-700">{amountLabel}</span>.</>
            : 'Your payment was received. Your order is being processed and will be delivered shortly.'}
        </p>
        {(amountLabel || trxref) && (
          <div className="bg-gray-50 rounded-xl p-4 mb-8 space-y-3">
            {amountLabel && (
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-widest font-bold mb-1">Amount Credited</p>
                <p className="text-sm text-gray-700 font-bold">{amountLabel}</p>
              </div>
            )}
            {trxref && (
              <div>
