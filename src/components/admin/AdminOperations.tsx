...
function StatCard({ label, value, sub, icon: Icon, tone = 'blue' }: { label: string; value: string; sub?: string; icon: any; tone?: 'blue' | 'red' | 'amber' | 'green' | 'gold' }) {
  ...
  return (
    <div className="admin-card p-3.5 min-h-[112px] flex flex-col justify-between">
      ...
      <div>
        <p className="text-[19px] font-extrabold text-admin-navy font-display leading-tight">{value}</p>
...
export default function AdminOperations() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Queue tabs */}
      <div role="tablist" aria-label="Transaction queue" className="flex bg-white p-1.5 rounded-xl border border-gray-100 shadow-sm w-fit overflow-x-auto">
        {([
      ...
      <div className="admin-card p-4 space-y-3">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button onClick={handleRefreshAll} disabled={isLoading} className="inline-flex items-center gap-1.5 px-2.5 py-2 bg-white border border-gray-200 rounded-lg text-[12px] font-bold text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50">
        ...
        {showFilters && (
          <div className="flex flex-wrap gap-3 pt-3 border-t border-gray-100">
            ...
            <div className="flex flex-col gap-1">
              ...
              <select value={providerFilter} onChange={(e) => setProviderFilter(e.target.value)} className="px-2.5 py-1.5 bg-gray-50 border border-gray-100 rounded-md text-[11.5px] font-semibold outline-none focus:ring-2 focus:ring-admin-blue capitalize">
            ...
            {tab === 'MANUAL_REVIEW' && (
              <div className="flex flex-col gap-1">
                ...
                <select value={reviewStatusFilter} onChange={(e) => setReviewStatusFilter(e.target.value)} className="px-2.5 py-1.5 bg-gray-50 border border-gray-100 rounded-md text-[11.5px] font-semibold outline-none focus:ring-2 focus:ring-admin-blue capitalize">
          ...
          <>
              ...
              <table className="w-full text-left min-w-[760px]">
                <thead>
                  <tr className="bg-gray-50/50 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">
                    {tab === 'FAILED' && <th className="px-3 py-2 w-10">
                    ...
                    {tab === 'FAILED' && <th className="px-3 py-2">Retries</th>}
                    {tab === 'MANUAL_REVIEW' && <th className="px-3 py-2">Review</th>}
                  ...
                  {txns.map((tx) => {
                    ...
                    return (
                      <tr key={tx._id} onClick={() => setDetailId(tx._id)} className="hover:bg-gray-50 transition-colors cursor-pointer">
                        ...
                        {tab === 'FAILED' && <td className="px-3 py-2 text-xs text-gray-500">{tx.retryCount || 0}</td>}
                        {tab === 'MANUAL_REVIEW' && (
                          <td className="px-3 py-2">
            ...
            <div className="md:hidden divide-y divide-gray-50">
              {txns.map((tx) => {
                const user = typeof tx.userId === 'object' ? tx.userId : null;
                return (
                  <button key={tx._id} onClick={() => setDetailId(tx._id)} className="w-full flex items-start gap-2.5 px-3 py-2 text-left active:bg-gray-50 transition-colors">
                    {tab === 'FAILED' && (
                      <input
