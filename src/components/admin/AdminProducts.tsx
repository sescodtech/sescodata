    ...
    <div className="space-y-4">
      ...
      <div className="admin-card p-4 space-y-3">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button onClick={handleExport} className="inline-flex items-center gap-1.5 px-2.5 py-2 bg-white border border-gray-200 rounded-lg text-[12px] font-bold text-gray-700 hover:bg-gray-50 transition-colors">
            <Download size={14} /> <span className="hidden sm:inline">Export</span>
          </button>
          <button onClick={() => fileInputRef.current?.click()} disabled={isImporting} className="inline-flex items-center gap-1.5 px-2.5 py-2 bg-white border border-gray-200 rounded-lg text-[12px] font-bold text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50">
                  ...
                  <tbody className="text-[12.5px] divide-y divide-gray-50">
                    {items.map((p) => (
                      <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-3 py-2 text-xs text-gray-600 capitalize">{p.provider}</td>
                        <td className="px-3 py-2 text-xs text-gray-500">{p.category === 'electricity' ? '—' : formatNaira(p.costPrice)}</td>
                        <td className="px-3 py-2 font-bold text-xs text-gray-900">{p.category === 'electricity' ? 'Amount-based' : formatNaira(p.sellingPrice)}</td>
                        <td className="px-3 py-2">
